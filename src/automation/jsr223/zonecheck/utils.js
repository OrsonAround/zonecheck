...
setDefaultItemValues(false, false);
        ...
        if (tags.contains('Required') || tags.contains('Sensor') >= 0) {
          ...
          if (!passed) {
            logger.warn(
              'FAILED: ' +
                member.getName() +
                ' has not been seen in the last five minutes.'
            );
          }
        }
      });
